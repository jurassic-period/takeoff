...
const makeContactsWithUniqueId = () =>
  INITIAL_DATA.map((el) => {
    const id = _.uniqueId();
    return { ...el, id };
  });
...
export default class Office extends PureComponent {
  ...
  getEditedCard = (card) => {
    ...
    if (uniqueId)
      return this.setState({ contacts: [...contacts, card], addCard: false });
  ...
  render() {
    ...
    if (edit)
      return <EditForm card={editedCard} getEditedCard={this.getEditedCard} />;
    if (addCard)
      return (
        <EditForm
          id={_.uniqueId()}
          cancel={this.cancel}
          getEditedCard={this.getEditedCard}
        />
      );
    return (
      <div className="row office">
        <SearchInput changeTerm={this.changeTerm} />
        <div className="col-12 office__title">
          Вы можете удалять, изменять или добавлять новые карточки с контактными
          данными
        </div>
        ...
        >
          Добавить карточку с адресом
        </button>
        <Link to="/" className="office__link">
